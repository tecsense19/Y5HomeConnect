...
import { ShoppingCart, RotateCcw, Plus, Minus, Layers, ClipboardList, Wifi, Home, Pencil, Trash2, ChevronDown, Check } from "lucide-react";
...
function PaletteIcon({ id }: { id: string }) {
  ...
  return (
    <div
      ...
      {...attributes}
      className={cn(
        "flex flex-col items-center justify-center p-2 rounded-lg border bg-background hover:bg-muted/50 cursor-grab active:cursor-grabbing hover:border-foreground/20 transition-all",
        isDragging && "opacity-50 ring-2 ring-primary ring-offset-1"
      )}
      title={def.label}
    >
      {def.image_path ? (
        <img crossOrigin="anonymous" src={def.image_path} alt={def.label} className="w-5 h-5 drop-shadow-sm opacity-80" style={{ objectFit: "contain", filter: "brightness(0)" }} />
      ) : def.icon ? (
...
function MasterSlot({
  ...
  const def = iconId ? ICON_MAP[iconId] : null;
  const Icon = def?.icon;
        ...
        <div className="pointer-events-none flex items-center justify-center w-full h-full">
          {Icon || def?.image_path ? (
            iconId === "power" ? (
              ...
              <img crossOrigin="anonymous" src={def.image_path} alt={def.label} className={iconClass} style={{ width: size * 5, height: size * 5, objectFit: "contain", filter: !isOn && glowClass?.includes("blue") ? "brightness(0) invert(0.6)" : (isOn ? "brightness(0) invert(1)" : "brightness(0)") }} />
            ) : Icon ? (
...
function CatalogCard({ preset, onPick, catalogColor, isSelected }: { preset: Preset; onPick: () => void; catalogColor: string; isSelected?: boolean }) {
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-center p-2 bg-secondary/10 rounded-lg border border-border shadow-inner min-h-[120px] w-full lighting-preview-box">
        {(() => {
          const panelDef = PANELS[preset.type];
          const scale = panelDef ? Math.min(120 / panelDef.frame.w, 85 / panelDef.frame.h) : 0.42;
...
function Kiosk() {
  ...
  const [openFrameCombo, setOpenFrameCombo] = useState(false);
  ...
  const [locations, setLocations] = useState<string[]>(LOCATION_TAGS);
  ...
  useEffect(() => {
    async function loadConfig() {
      try {
        ...
        const [locRes, iconRes, colorRes] = await Promise.all([
          fetch(`${apiUrl}/locations`),
          fetch(`${apiUrl}/icons`),
          fetch(`${apiUrl}/frame-colors`),
        ]);

        if (locRes.ok) {
          const locData = await locRes.json();
          if (locData && locData.length > 0) {
            setLocations(locData.map((l: any) => l.name));
          }
        }
        ...
        if (iconRes.ok) {
          ...
          if (iconData && iconData.length > 0) {
            const originalIconMap = { ...ICON_MAP };
            ICONS.length = 0;
            Object.keys(ICON_MAP).forEach(k => delete ICON_MAP[k]);
            iconData.forEach((i: any) => {
              const iconId = i.name.toLowerCase().replace(/[^a-z0-9]/g, '-');
              const isSystem = i.image_path && i.image_path.startsWith('system:');
              const mapId = isSystem ? i.image_path.replace('system:', '') : iconId;
        ...
        if (colorRes.ok) {
          ...
          if (colorData && colorData.length > 0) {
            ...
            colorData.forEach((c: any) => {
              let s = "classic";
              if (c.series === "architectural-elite") s = "elite";
              else if (c.series === "architectural-pro") s = "pro";
              else if (c.series === "architectural-pro-plus") s = "pro-plus";
              else if (c.series === "classic") s = "classic";

              const colorId = c.name.toLowerCase().replace(/[^a-z0-9]/g, '-');
              if (seriesColorMap[s]) {
                seriesColorMap[s].push(colorId);
              }

              const existing = COLOR_THEMES.find(t => t.id === colorId);
              if (!existing && c.hex_code) {
                newThemes.push({
                  id: colorId,
                  name: c.name,
                  bg: c.hex_code,
                  ...
                  btnBg: c.hex_code,
                  text: "#FFFFFF",
                  previewBg: c.hex_code,
                });
              }
            });

            Object.keys(SERIES_COLORS).forEach(k => delete SERIES_COLORS[k]);
            Object.assign(SERIES_COLORS, seriesColorMap);

            if (newThemes.length > 0) {
              COLOR_THEMES.push(...newThemes);
            }
          }
        }
      } catch (err) {
  ...
  function changeSeries(seriesId: string) {
    ...
    cfg.color = allowed.includes(master.color || "black") ? (master.color || "black") : allowed[0];
    setMaster(cfg);
  ...
  function loadPreset(p: Preset) {
    ...
    const allowed = SERIES_COLORS[selectedSeries] || ["black"];
    const targetColor = master.color || "black";
    cfg.color = allowed.includes(targetColor) ? targetColor : allowed[0];
    setMaster(cfg);
    setQty(p.qty);
  ...
  function addToCart() {
    ...
    if (!master.location?.trim()) {
      toast.error("Please select or enter a Location / Room Tag");
      return;
    }
        ...
        <div className="relative z-10">
          ...
          <div className="grid grid-cols-1 gap-4 p-4 lg:grid-cols-[1fr_380px]">
            ...
            <main className="rounded-xl border border-border bg-background p-6 shadow-sm">
              ...
              <div className="grid grid-cols-1 gap-x-2 gap-y-6 sm:grid-cols-2 md:grid-cols-3 2xl:grid-cols-4">
      ...
      <div className="min-h-screen bg-background/95 dark:bg-zinc-950/90 relative">
        ...
        <div className="relative z-10">
          ...
          <div className="grid grid-cols-1 gap-4 p-4 lg:grid-cols-[1fr_380px]">
            {/* ===================== Catalog (left/main) ===================== */}
            <main className="rounded-xl border border-border bg-background p-6 shadow-sm">
              <div className="mb-6 flex flex-col 2xl:flex-row 2xl:items-center justify-between gap-6 border-b pb-6">
                ...
                {selectedSeries === "classic" && (
                  <div className="grid grid-cols-2 sm:flex sm:grid-cols-none bg-secondary p-1 rounded-lg w-full sm:w-auto self-center shrink-0 gap-1 sm:gap-0">
                    <button
                      onClick={() => setActiveTab("frames")}
                      className={cn(
                        "flex items-center justify-center gap-1.5 px-2 sm:px-3 py-1.5 text-[11px] sm:text-xs font-semibold rounded-md transition",
                        activeTab === "frames"
                          ? "bg-[#034B25] text-white shadow-sm"
                          : "text-muted-foreground hover:text-foreground",
                      )}
                    >
                      <Layers size={14} className="shrink-0" /> <span className="truncate">Unique Frames</span>
                    </button>
                    <button
                    ...
                    >
                      <ClipboardList size={14} className="shrink-0" /> <span className="truncate">Presets</span>
                    </button>
                  </div>
                )}
              </div>
                ...
                <div className="space-y-10">
                  {categories.map((cat) => {
                    ...
                    const renderPanelCard = (p: typeof categoryPanels[0]) => {
                      ...
                      // Always render the left panel catalog thumbnails as white/neutral for visibility
                      const catalogColor = "white";
                      const config = {
                ...
                {true ? (
                  <>
                    <div className="mb-3 flex flex-col gap-2">
                      ...
                      {/* Searchable Dropdown Frame Selector */}
                      <Popover open={openFrameCombo} onOpenChange={setOpenFrameCombo}>
                        <PopoverTrigger asChild>
                          <Button
                            variant="outline"
                            role="combobox"
                            aria-expanded={openFrameCombo}
                            className="w-full justify-between h-auto py-2 px-2.5 text-[11px] font-semibold uppercase tracking-wider bg-background text-left hover:border-destructive/50 transition whitespace-normal"
                          >
                            <span className="line-clamp-2 text-left leading-tight pr-2">
                              {PANELS[masterType]?.label || "Select frame..."}
                            </span>
                        ...
                        <PopoverContent className="w-[340px] p-0" align="start">
                          <Command>
                            <CommandInput placeholder="Search frames..." className="h-9 text-xs uppercase" />
                            <CommandList>
                              ...
                              {categories.map((cat) => {
                                ...
                                return (
                                  <CommandGroup key={cat} heading={`${cat} Frames`}>
                                    {frames.map((p) => (
                                      <CommandItem
                                        key={p.id}
                                        value={p.label}
                                        onSelect={() => {
                                          changeType(p.id);
                                          setOpenFrameCombo(false);
                                        }}
                                        className="text-xs uppercase cursor-pointer py-2"
                                      >
                                        <div className="flex flex-1 pr-2 font-medium">
                                          {p.label}
                                        </div>
                                        <Check
                                          className={cn(
                                            "ml-auto h-4 w-4 shrink-0",
                                            masterType === p.id ? "opacity-100 text-destructive" : "opacity-0"
                                          )}
                                        />
                                      </CommandItem>
                                    ))}
                    ...
                    {/* qty + actions */}
                    <div className="mt-4 flex flex-row items-center justify-between gap-2 w-full overflow-x-auto scrollbar-hide pb-1">
                      <div className="flex items-center gap-1 rounded-md border border-border bg-background px-1 h-9 shrink-0">
                        <button
      ...
      <DragOverlay dropAnimation={null}>
        {dragDef ? (
          <div className="flex aspect-square w-14 items-center justify-center rounded-xl border-2 border-amber-500/80 bg-card/95 backdrop-blur-sm shadow-xl p-1 text-center">
            {dragDef.image_path ? (
              <img crossOrigin="anonymous" src={dragDef.image_path} alt={dragDef.label} className="w-7 h-7 drop-shadow-sm opacity-80" style={{ objectFit: "contain", filter: "brightness(0)" }} />
            ) : DragIconComp ? (
